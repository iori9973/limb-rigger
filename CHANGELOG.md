...
### Fixed
- Constraint 方式（回転のみ）で配置が保持されず再生時にボーンがスナップしてずれる問題を修正。rest オフセットを効かない `ParentRotationOffset`（ソースごとの欄）ではなく、Rotation Constraint が実際に使う `RotationOffset`（コンストレイント単体の欄）に焼き込むよう変更。これにより指など「回転のみで自分のピボットを中心に曲げたい」用途が正しく動作する
